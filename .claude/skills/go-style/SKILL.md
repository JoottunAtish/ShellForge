...
| `golang.org/x/sys` | Windows console mode | Go team maintained golang.org/x module; approved for #68, `internal/platform`'s `GetConsoleMode`/`SetConsoleMode` calls for virtual terminal processing. Already present as an indirect transitive of `golang.org/x/term`; this only adds a direct import at the pinned v0.34.0, no version change. |
