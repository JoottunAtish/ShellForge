...
func (rt *dockerRuntime) containerIsStale(ctx context.Context, image string, st containerState) (bool, error) {
	...
	if err != nil {
		return false, rt.classifyFailure(ctx, "inspect the sandbox image", err, combinedOutput(stdout, stderr))
	}
	if code != 0 {
...
func (rt *dockerRuntime) createContainer(ctx context.Context, image string) error {
	...
	stdout, stderr, code, err := rt.run.run(ctx, argv, nil)
	if err != nil {
		return rt.classifyFailure(ctx, "start the sandbox container", err, combinedOutput(stdout, stderr))
	}
	if code != 0 {
...
func (rt *dockerRuntime) inspectContainer(ctx context.Context) (containerState, error) {
	...
	if runErr != nil {
		return containerState{}, rt.classifyFailure(ctx, "inspect the sandbox container", runErr, combinedOutput(stdout, stderr))
	}
	if code != 0 {
