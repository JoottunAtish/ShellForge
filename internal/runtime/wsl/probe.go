...
// PATH, runs `wsl -l -v` through the package's existing runner, and parses
// the result with the package's existing decoder. A non-zero exit whose
// stdout does not parse as a distribution table is read the same forgiving
// way listRows reads it: as "no distributions registered", not a failure,
// because there is nothing importing yet for this probe to have broken.
func realVersion2Probe(ctx context.Context) ([]distro, error) {
	...
	stdout, _, code, err := r.run(ctx, []string{"wsl.exe", "-l", "-v"}, nil)
	if err != nil {
		return nil, err
	}
