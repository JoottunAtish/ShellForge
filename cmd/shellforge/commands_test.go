...
func TestUnimplementedVerbsExplainThemselves(t *testing.T) {
	...
	// of the Day 4 game core. `bug-report` is real as of #158: it gathers a
	// diagnostic bundle through internal/bugreport instead of refusing.
	// All of these are deliberately absent above rather than deleted from
	// the documented set: author_test.go covers validate,
	// author_test_cmd_test.go covers test, cmd_doctor_test.go covers
	// doctor, cmd_init_test.go and cmd_sandbox_test.go cover init and
	// sandbox, render_map_test.go covers map's rendering, and
	// cmd_bugreport_test.go covers bug-report. A real verb still errors
